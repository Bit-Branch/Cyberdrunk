...
public class InfiniteBackground : MonoBehaviour{
    ...
    void loadChildObjects(GameObject obj){
        ...
        for(int i = 0; i <= childsNeeded; i++){
            ...
            c.transform.position = new Vector3(obj.transform.position.x,objectWidth * i , obj.transform.position.z+1);
            c.name = obj.name + i;
        }
        Destroy(clone);
    ...
    void repositionChildObjects(GameObject obj){
        ...
        if(children.Length > 1){
            ...
            if(transform.position.y + screenBounds.y > lastChild.transform.position.y + halfObjectWidth){
                ...
                firstChild.transform.position = new Vector3(lastChild.transform.position.y,  lastChild.transform.position.y + halfObjectWidth * 2, lastChild.transform.position.z);
